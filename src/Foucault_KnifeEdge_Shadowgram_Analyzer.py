...
def find_matching_intensities_and_draw_lines(lst, x1, y1, r1, tolerance, image, line_length, save_plot, plot_output, closest_match_threshold, plot_legend, mirror_diameter_inches, mirror_focal_length_inches):
    ...
    for i in matches:
        ...
        if abs(int(i[0][2])-int(i[1][2])) < closest_match_threshold:
           ...
           draw_text(image, f"Mirror Focal Length: {mirror_focal_length_inches:.2f}\"", (x1-20,y1+r1-60), font=cv2.FONT_HERSHEY_SIMPLEX, font_scale=0.3, color=(255, 255, 255), thickness=1)
           ...
           draw_text(image, f"Closest Match Threshold: {closest_match_threshold}", (x1-20,y1-r1+40), font=cv2.FONT_HERSHEY_SIMPLEX, font_scale=0.3, color=(255, 255, 255), thickness=1)
           draw_text(image, f"Brightness Tolerance: {tolerance}", (x1-20,y1-r1+20), font=cv2.FONT_HERSHEY_SIMPLEX, font_scale=0.3, color=(255, 255, 255), thickness=1)
